...
namespace WindowsApplication1
{
	...
	public class COMsetup : Form
	{
		...
		private void COMsetup_Load(object sender, EventArgs e)
		{
			...
			for (i = 1; i <= num1; i = (void*)(checked((ushort)(i + 1))))
			{
				this.COMportNumber.Items.Add(string.Concat("APG USB to LIN ", num.ToString()));
				num = checked(num + 1);
			}
			num = 0;
			ushort num2 = Basic.How_Many_Of_MyDevices_Are_Attached(MyProject.Forms.Network.PKSAPID);
			for (i = 1; i <= num2; i = (void*)(checked((ushort)(i + 1))))
			{
				this.COMportNumber.Items.Add(string.Concat("PKSA ", num.ToString()));
				num = checked(num + 1);
			}
			if (MyProject.Forms.Network.SelectedCOMport == -1)
			{
				this.COMportNumber.Text = "No USB device present";
				this.TimeoutBox.Text = "Not Available";
			}
			else
			{
				this.COMportNumber.Text = Conversions.ToString(this.COMportNumber.Items[MyProject.Forms.Network.SelectedCOMport]);
